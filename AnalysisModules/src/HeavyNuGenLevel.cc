...
void HeavyNuGenLevel::HistStruct::book(TFileDirectory *td, const std::string& post, float* pw)
{
    ...
    t = "MJJ : " + post;
    mjj = td->make<TH1F > ("mJJ", t.c_str(), 200, 0, 2000);
    t = "MWR : " + post;
    m4obj = td->make<TH1F > ("m4obj", t.c_str(), 400, 0, 4000);
}
...
void HeavyNuGenLevel::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    if (iEvent.getByLabel("generator", hepMCEvt))
    {
        ...
        //    return;
        //}
        if(theLep) l1 = reco::Particle::LorentzVector(theLep->momentum().px(), theLep->momentum().py(), theLep->momentum().pz(), theLep->momentum().e());
        else       l1 = reco::Particle::LorentzVector(0.0, 0.0, 0.0, 0.0);
        if(theLep2) l2 = reco::Particle::LorentzVector(theLep2->momentum().px(), theLep2->momentum().py(), theLep2->momentum().pz(), theLep2->momentum().e());
        else        l2 = reco::Particle::LorentzVector(0.0, 0.0, 0.0, 0.0);
    ...
    else
    {
        ...
        if(nfound < 1) l1 = reco::Particle::LorentzVector(0.0, 0.0, 0.0, 0.0);
        if(nfound < 2) l2 = reco::Particle::LorentzVector(0.0, 0.0, 0.0, 0.0);
